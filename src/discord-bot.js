    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        try {
            switch (interaction.commandName) {
                ...
                case 'vote': await handleVote(interaction); break;
...
async function handleVote(interaction) {
    const embed = new EmbedBuilder()
        .setColor(0xF77F00)
        .setTitle('Vote for GearAlert Bot!')
        .setDescription('Voting helps us rank higher on Top.gg so more musicians can find deals.')
        .addFields(
            { name: 'Vote Now', value: '[Click here to vote on Top.gg](https://top.gg/bot/1487362132289781780/vote)' },
            { name: 'Why Vote?', value: 'Every vote helps the bot reach more gear hunters!' },
        )
        .setTimestamp();
